...
* Run on your own public cloud (BYOC)
* Run on StarTree Cloud
** This can be using the FREE tier or paid tier
...
_Real Time servers can also load segments from Segment Store_
...
Sample command to deploy Zookeeper:

====
[source]
docker run --network=pinot-advanced  --name pinot-zookeeper --restart always -p 2181:2181 -d zookeeper:3.5.6
====
...
----
pinot-controller:
    image: apachepinot/pinot:1.1.0                         <1>
...
Sample command to deploy Pinot Controller:

====
[source]
docker run -d --name pinot-controller --network pinot-advanced apachepinot/pinot:1.0.0 StartController -zkAddress pinot-zookeeper:2181
====
...
Sample command to deploy Pinot Server:
...
|https://github.com/startreedata/learn/pinot-advanced/01-create-cluster
...
Typical use case might be segregating data and compute by department within a company or critical and non-critical tasks.
...
a|image::images/link.png[]
|https://github.com/startreedata/learn/pinot-advanced/10-add-tenat
a|image::images/check.png[]
...
* Deep store is used for backup and restore operations, as well as replication.  It is sometimes used to move data.
...
* Tables can bypass the controller and load data directly into the deep store by setting _realtime.segment.serverUploadToDeepStore=true_

== Data Flow in Offline Segments

image::images/data-flow-2.png[]

In the case of Batch ingestion, the data is written directly to the Deep Store when configured.
...
a|*API* +
a|*UI* +
a|image::images/create-table-1.png[]
a|image::images/create-table-2.png[]
a|image::images/create-table-3.png[]
|===
...
* Dimension tables are used for lookups.  
* The cannot be HYBRID or REALTIME.  
* They are replicated on all hosts and loaded in memory, for faster lookups.  
* They are typically small (limit 200MB).
...
a|image::images/link.png[]
|https://github.com/startreedata/learn/pinot-advanced/02-add-table
a|image::images/check.png[]
...
* Data can be ingested from disk, spark, Hadoop and cloud.
...
Batch Data can be ingested using the following methods:

* CLI using JobSpec
* UI - for small, test datasets
* API - for adding data programatically
* Minion Tasks - preferred method for production wokloads
...
<6> Output directory
<7> File system to use
<8> Record Reader Specification
<9> Table Specification
<10> Segment Name Generator Spec
<11> Pinot Cluster Specification
<12> Push Job Specification
====
...
* This method is for testing, not production ready workloads.
...
a|image::images/link.png[]
|https://github.com/startreedata/learn/pinot-advanced/03-batch-ingestion
a|image::images/check.png[]
...
* When resuming, you can specify an additional parameter _resumeFrom=smallest/largest_
====
[source, bash]
----
...
a|image::images/link.png[]
|https://github.com/startreedata/learn/pinot-advanced/04-stream-ingestion
a|image::images/check.png[]
...
The Pinot single-stage query engine (also known as v1 query engine) uses a scatter-gather query engine model.

image::images/v1-query.png[]

== Multi Stage Query Engine

* Multi-stage query engine (v2) allows for querying distributed joins, window functions, and other multi-stage operators in real-time.
* It addresses some of the bottlenecks on v1 query engine.

image::images/v2-query.png[]
...
You can view the query plan in verbose or non-verbose (default) mode.  Verbose adds additional information to the query plan such as filters with and without indexes.

To enable verbose mode the query option _explainPlanVerbose=true_ must be passed.
...
== Query Plan Multi Stage

In Pinot 1.0 and above, you have the option of querying the logical plan or the implementation plan for a given query using the EXPLAIN PLAN syntax
...
a|image::images/link.png[]
|https://github.com/startreedata/learn/05-pinot-advanced/query
a|image::images/check.png[]
...
a|image::images/link.png[]
|https://github.com/startreedata/learn/pinot-advanced/06-upsert
a|image::images/check.png[]
...
* Groovy functions are disabled by default
* To use Groovy functions, modify controller config to enable it set disabled = false:
** _controller.disable.ingestion.groovy=false_
* Example: Use the syntax shown here to concatenate names.
...
----
<1> Groovy function to concatenate string
====
...
a|image::images/link.png[]
|https://github.com/startreedata/learn/pinot-advanced/07-transform
a|image::images/check.png[]
...
* On Table Creation
* Dynamically, by editing the Table Configuration (except for sorted index)

_Sorted Indexes include Forward and Inverted Sorted Index_
...
* The star-tree index allows for pre-aggregation of most used data, and combines seldom queried data into "Star" buckets
...
* To use JSON index, use the JSON_MATCH function.
...
* Supports Vector Index on float array/multi-value columns
* Add predicate and function to retrieve topK closest vector
...
a|image::images/link.png[]
|https://github.com/startreedata/learn/pinot-advanced/06-indexes
a|image::images/check.png[]
...
* Converts Real-time segments to Offline segments
* Automatically manage the movement of the data
* Moving to Offline allows for deduplication, backfill, and querying bigger windows of data.
